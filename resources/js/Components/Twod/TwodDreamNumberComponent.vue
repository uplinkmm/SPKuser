<template>
    <div class="w-full lg:w-7/12 mx-auto px-4 bg-white pb-8 min-h-[100vh]">
        <div class="flex justify-between py-4 px-0 lg:px-4 mb-4">
            ...
            <p class="text-black text-base">အိပ်မက်တစ်ရာ</p>
            <div class="flex gap-x-4">
                <div class="relative">
                    ...
                    <label
                        for="search"
                        class="relative rounded-md border overflow-hidden block"
                    >
                        <input
                            type="text"
                            id="search"
                            class="border-0 px-1 py-1 text-sm"
                            @input="searchDream"
                            v-model="searchInput"
                        />
                            ...
                            >
                                <thead class="font-medium text-black">
                                    <tr>
                                        <th
                                            scope="col"
                                            class="px-3 md:px-6 py-4 font-normal"
                                        >
                                            စဉ်
                                        </th>
                                        <th
                                            scope="col"
                                            class="py-3 font-normal whitespace-nowrap"
                                        >
                                            အိပ်မက်
                                        </th>
                                        <th
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td class="py-4">
                                            <img
                                                class="w-12 lg:w-16 aspect-square"
                                                :ref="'image_' + dNumber.id"
                                                :src="placeholder"
                                                @load="
                                                    loadDreamImage(
                                                        dNumber.image_url,
    ...
    methods: {
        async getDreamNumberList() {
            const response = await getApiData({
                url: "http://128.199.218.198:83/api/dream_numbers",
            });
            if (response.data) {
        ...
        async searchDream() {
            const response = await getApiData({
                url:
                    "http://128.199.218.198:83/api/dream_numbers?search=" +
                    this.searchInput,
            });
            if (response.data) {
                this.dreamNumberList = response.data;
