...
trait GscWebhook
{
    ...
    /**
     * @param  array<int,GscRequestTransaction>  $requestTransactions
        ...
        foreach ($requestTransactions as $requestTransaction) {
            ...
            $wager = Wager::updateOrCreate(
                ...
                [
                    'customer_id' => $event->customer_id, //change from  'user_id'=> $event->user_id,
                    'seamless_wager_id' => $requestTransaction->wager_code,
                    'wager_type' => $requestTransaction->wager_type,
                    'wager_status' => $requestTransaction->wager_status,
                    // 'seamless_transaction_id' => $requestTransaction->TransactionID,
                ]
            );
            // }
            if ($refund) {
                ...
                $wager->update([
                    'status' => $requestTransaction->amount > 0 ? WagerStatus::Win : WagerStatus::Lose,
                ]);
            }
            $game_type = GameType::where('code', $requestTransaction->game_type)->first();
            if (!$game_type) {
                throw new Exception("Game type not found for {$requestTransaction->game_type}");
            }
            $product = Product::where('code', $requestTransaction->product_code)->first();
            if (!$product) {
                throw new Exception("Product not found for {$requestTransaction->product_code}");
            }
            ...
            $seamless_transactions[] = $event->transactions()->create([
                ...
                'bet_amount' => $requestTransaction->bet_amount,
